...
public:
    Random(cRNG *rng, JobDispatcher* job_dispatcher, bool force_distributed=false, bool force_multi_racks=false)
    : rng(rng), job_dispatcher(job_dispatcher), force_distributed(force_distributed), force_multi_racks(force_multi_racks){}
    ...
    std::unordered_map<int, unsigned> place_job(const Job* job) override {
        ...
        for (auto pair : job_dispatcher->workers) {
            ...
            if (job_dispatcher->free_gpus[wid] > 0) {
                available_machines++;
                tors_with_available_machines.insert(job_dispatcher->tor_id_for_worker[wid]);
            }
        ...
        auto must_place_distributed = force_distributed && available_machines > 1 && job->getGpu() > 1;
        auto must_place_multi_racks = force_multi_racks && tors_with_available_machines.size() > 1 && job->getGpu() > 1;
        do {
            do {
