...
void TrainingProcess::activity() {
    ...
    setup(job);
    worker->adjust_free_gpus(job->getGpu(), false); // decrement
    ...
    EV
              << fmt::format("rank {} done job {} at {}\n", rank, jid,
                      simTime().raw());
    worker->adjust_free_gpus(job->getGpu(), true); // increment free gpus
    job->setFinish_time(simTime());
    EV << "Finished Job " << jid << " as rank " << rank << " at " << simTime()
              << endl;
...
void TrainingProcess::setup(Job *job) {
    ...
    auto m = std::string(job->getModel());
    if (m == "alexnet") {
